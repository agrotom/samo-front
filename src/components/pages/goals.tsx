import TextHeader, { EditableTextHeader } from "@/components/atoms/text-header";
...
import Sticker from "../organisms/sticker";
...
import useObjectivesController, { type RawObjectiveData } from "@/controller/objectivesController";
import useDidUpdateEffect from "@/util/useDidUpdateEffect";
import type { ObjectiveGoalData } from "@/api/objective";
import { GoPlus } from "react-icons/go";
import { SortableCheckbox } from "../atoms/checkbox";
...
interface ObjectiveGoalProperties {
    objectiveId: number;
    data: ObjectiveGoalData,
    editable: boolean;
...
export function ObjectiveGoal({ objectiveId, data, editable, modifyObjectiveGoal, removeObjectiveGoal } : ObjectiveGoalProperties) {
    
    const text = useRef<string>(data.text);
    const completed = useRef<boolean>(data.completed);

    const [deadline, setDeadline] = useState<Date>(data.deadLine);
    const [deadlineOpen, setDeadlineOpen] = useState<boolean>(false);

    const modify = (date: Date, text: string, completed: boolean) => {
        modifyObjectiveGoal(objectiveId, data.id, { ...data, deadLine: date, completed: completed, text: text })
    };

    return (
        <div key={`objectiveGoal-${data.id}`} className="flex flex-row">
            <SortableCheckbox id={data.id} initText={data.text} onDelete={ () => removeObjectiveGoal(objectiveId, data.id) } value={data.completed} disabled={!editable} onChange={(checkboxData) => { text.current = checkboxData.text; completed.current = checkboxData.value; modify(deadline, checkboxData.text, checkboxData.value); }} />
            <div className="flex flex-row ml-auto">
                <HiClock className='size-5' />
                <div className="-mt-0.5">
                    <p className={ `${editable && 'cursor-pointer'}` } onClick={ () => { if (editable) setDeadlineOpen(true) } }>
                        { `${deadline.toLocaleDateString("ru-RU", { weekday: 'long' }).replace(/^./, (ch) => ch.toUpperCase())}, ${format(deadline, "d MMMM", {locale: ru})}` }
                    </p>
                </div>
            </div>
            <Calendar onChange={ date => { setDeadline(date); modify(date, text.current, completed.current); } } currentDate={ deadline } setOpen={ setDeadlineOpen } open={ deadlineOpen } />
        </div>
    )
}
...
export function ObjectiveModal({ openModal, setOpenModal }: ObjectiveModalProperties) {
    ...
    const modifyObjectiveGoal = useObjectivesController((state) => state.modifyObjectiveGoal);
    const removeObjectiveGoal = useObjectivesController((state) => state.removeObjectiveGoal);
    ...
    const gatherData = (): RawObjectiveData => {
        return { imageFile: uploadedImage, label: label, text: text, favorite: favorite, balanceType: balance, date: date, completed: completed, goalsLabel: goalsLabel, goals: goals };
    }

    const calcGoalsPercent = (): string => {
        ...
        return (completedGoals / allGoals * 100).toFixed(0);
    }
        ...
        <Modal open={openModal} setOpen={setOpenModal} onSubmit={ () => setCompleted(!completed) } submitText={ `${ completed ? 'Не сделано' : 'Сделано' }` } cancelText='' >
            <div className="flex flex-col relative">
                <div className="flex flex-row">
                    <EditableTextHeader text={label} editable={editable} onChange={ setLabel } />
                    <div className="flex flex-row ml-auto space-x-3">
                        ...
                        <button className='cursor-pointer' onClick={() => setEditable(!editable)}>
                            {
                                editable ?
                                <FaCheck className="text-active-bar  size-4 mx-1"/> :
                ...
                <div className="flex flex-row items-center space-x-2 pt-2"> 
                    ...
                    <div className="bg-inactive w-full h-2 my-auto rounded-lg">
                        <div style={{ width: `${ calcGoalsPercent() }%` }} className="bg-active-bar-alt h-2 my-auto rounded-lg" />
                    </div>
                    <button className="cursor-pointer" onClick={ () => addEmptyObjectiveGoal(loadedData.id) }>
                        <GoPlus className="size-5"/>
                    </button>
                </div>
                <div className="flex flex-col pl-5 pt-5 space-y-2">
                    {
                        loadedData.goals.map(data => <ObjectiveGoal objectiveId={loadedData.id} data={data} removeObjectiveGoal={removeObjectiveGoal} modifyObjectiveGoal={modifyObjectiveGoal} editable={editable} />)
                    }
                </div>
                ...
                <div onClick={ () => setShowenList(true) } className="flex mt-15 hover:bg-brand-active h-10 items-center rounded-sm w-full cursor-pointer px-2">
                    <button className="flex flex-row w-full items-center cursor-pointer">
                        ...
                        <p className="ml-5"><FormattedMessage id="sphere" />: <FormattedMessage id={balance.id} /></p>
    ...
    return ( 
        <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-6 h-full grid-rows-[auto_1fr]">
                <div className="relative bg-brand  p-6 rounded-lg md:col-span-3 h-50 md:h-fit overflow-hidden">
                    <div className="flex-row items-center space-x-10 hidden md:flex">
                        <TextHeader text="mission" />
                    ...
                    <BlockInput header="mission" initText={ mission } onChange={ text => setMission(text) } classNames={{ block: "md:hidden" }} />
                </div>
                <div className="relative bg-brand  p-6 rounded-lg overflow-hidden md:col-span-3 h-full">
                    <div className="lg:flex lg:flex-row lg:items-start md:h-full lg:overflow-auto">
                        <div className="grid grid-cols-1 lg:grid-cols-3 grid-rows-1 text-center w-full space-y-8 lg:space-y-0">
                                <div className="flex flex-col gap-4 h-150 overflow-auto">
                                    <TextHeader text="two_month" />
                                    {
                ...
                <div className="relative bg-brand  p-6 rounded-lg row-start-2 md:col-start-4 h-fit overflow-hidden">
                    <TextHeader text="focused_objectives" />
