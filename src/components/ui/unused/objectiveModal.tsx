...
export default function ObjectiveModal({ header = '', children, onCancel, onSubmit, open, setOpen }: ModalProperties) {

    return (
        <Dialog open={open} onClose={setOpen} className="relative z-20 text-primary dark:text-primary-dark">
                <DialogBackdrop
                transition
                ...
                <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    className="relative transform overflow-hidden rounded-lg bg-brand dark:bg-brand-dark text-left shadow-xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-lg data-closed:sm:translate-y-0 data-closed:sm:scale-95"
                        >
                        <div className="px-6 py-5 border-b-2 border-b-brand-active-dark">
                            <TextHeader text={header} />
                        </div>
                        <div className="px-4 pb-10 sm:p-6 sm:pb-10 overflow-y-auto">
                            {
                                children
                            }
                        </div>
                        <div className="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 border-t-2 border-t-brand-active-dark">
                            <button
                            type="button"
