        ...
        <>
            <TextHeader text={header} />
            {
                <div className={`flex flex-row relative overflow-hidden h-full mt-5`}>
                    <textarea ref={textAreaRef} value={ text } onChange={ value => setText(value.target.value) } className={ `border-active-bar  border-2 p-2 outline-0 rounded-lg w-full h-[75%] box-border shadow-inner resize-none text-wrap break-all ${ !outerControl && 'hidden' } ${ children && 'mr-5' }` }/>
        ...
        <div className={`flex flex-col h-full ${classNames.block}`}>
            ...
            }
            {
                <div className={`flex flex-row relative box-border overflow-y-auto overflow-x-clip h-full min-h-0 mt-1 ${classNames.container}`}>
                    {
                        isEditMode() ?
                        <textarea onBlur={ () => { if (!outerControl) setEditable(false); onChange?.(text); } } ref={textAreaRef} value={ text } onChange={ value => setText(value.target.value) } className={ `block border-active-bar  border-2 p-2 outline-0 rounded-lg w-full h-full box-border shadow-inner resize-none text-wrap break-all ${ children && 'mr-5' } ${classNames.textarea}` }/> :
